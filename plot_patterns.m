% h = plot_patterns(patterns, chanlocs, varargin)
%
...
function h = plot_patterns(patterns, chanlocs, varargin)
...
for p = 1:size(patterns, 2)
    ...
    % calling topoplot
    if samescale, evalc('topoplot(patterns(:,p), chanlocs, ''maplimits'', [scalemin, scalemax])');
    else,         evalc('topoplot(patterns(:,p), chanlocs)'); end
